...
class FakeProvider:
    ...
    async def stream_generate(self, messages: List[Dict[str, Any]]) -> AsyncIterator[Dict[str, Any]]:  # noqa: ANN401
        yield {"type": EventType.TOKEN_EMITTED.value, "data": {"role": "assistant", "delta": "Hi"}}
...
async def test_llm_provider_contract():
    ...
    frames = []
    async for frame in provider.stream_decision([{"role": "user", "content": [{"type": "text", "data": "hi"}]}], schema=None):
        frames.append(frame)
