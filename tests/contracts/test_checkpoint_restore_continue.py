...
async def test_restore_and_continue_flow():
    ...
    await orch.control(
        session_id=session.id, command={"type": "checkpoint.requested", "id": "cp2"}
    )
    await orch.control(
        session_id=session.id, command={"type": "checkpoint.restore", "id": "cp2"}
    )
    ...
    async def consume_second():
        ...
        async for ev in orch.stream(
            ...
            inputs={
                "messages": [
                    {"role": "user", "content": [{"type": "text", "data": "go2"}]}
                ]
            },
        ):
            if ev.type == EventType.DECISION_COMPLETED:
