...
async def test_checkpoint_restore_and_replay_projection():
    ...
    await orch.control(session_id=session.id, command={"type": "checkpoint.requested", "id": "cp1"})
    # restore checkpoint
    await orch.control(session_id=session.id, command={"type": "checkpoint.restore", "id": "cp1"})
